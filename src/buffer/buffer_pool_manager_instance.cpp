...
Page *BufferPoolManagerInstance::NewPgImp(page_id_t *page_id) {
  std::scoped_lock clock_lock{latch_};
  // if (pool_size_ == page_table_.size()) {
  //   return nullptr;
  // }
  ...
  *page_id = AllocatePage();
  ...
  // 这里不需要这个逻辑，因为我一定是先从free_list里找，如果freelist没有，再去Replacer里找
  // Replacer里面放了所有Unpin的frame，如果Replacer是空的，就代表所有frame都pinned
...
Page *BufferPoolManagerInstance::FetchPgImp(page_id_t page_id) {
  ...
  if (free_list_.empty()) {
    ...
    // 这里的刷脏逻辑最好不要写在外层if，因为外层有可能是freelist的frame，而freelist出来的frame肯定不用刷盘
